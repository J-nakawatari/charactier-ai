...
import { MessageCircle, Clock, TrendingUp, Heart, Calendar, Zap } from 'lucide-react';
...
export default function ChatAnalyticsPage() {
  ...
  useEffect(() => {
    const fetchChatAnalytics = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/analytics/chats?range=${timeRange}`, {
          headers: {
        ...
        // フォールバック: モックデータを使用
        setAnalyticsData(generateMockChatAnalytics());
      } finally {
        setIsLoading(false);
      }
    };
