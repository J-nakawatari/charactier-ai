...
test.describe('99% Profit System E2E Test', () => {
  ...
  test('Token pack creation with 99% profit rate verification', async ({ page }) => {
    // Navigate to token management - Japanese text in has-text is OK
    ...
    // Intercept API response to verify profit rate
    ...
    if (responseData.tokenPack) {
      const costRatio = 0.01; // 1% cost
      const expectedMinTokens = Math.floor(testPrice * costRatio * 0.9); // with some margin
  ...
  test('Price recalculation on exchange rate change', async ({ page }) => {
    ...
    // Click edit button for created pack
    ...
    // Verify 99% profit rate is maintained
    const ratio = parseInt(newTokenAmount) / parseInt(originalTokenAmount);
    const expectedRatio = 3000 / 2000;
    expect(Math.abs(ratio - expectedRatio)).toBeLessThan(0.1); // within 10% error
  });

  test('Stripe Price ID accurate registration and retrieval', async ({ page }) => {
    // Navigate to token management
    await page.locator('a:has-text("トークチケット管理")').click();
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    ...
    // Expect Stripe Price ID auto generation
    const responsePromise = page.waitForResponse(
      response => response.url().includes('/api/v1/admin/token-packs') && response.request().method() === 'POST'
    );
    ...
    // Verify Stripe Price ID is set correctly
  ...
  test('Edge cases for profit rate calculation', async ({ page }) => {
    ...
    // Test with small amount (100 yen)
    ...
    // Test with large amount (10000 yen)
    ...
    expect(Math.abs(ratio - expectedRatio)).toBeLessThan(1); // error less than 1
  });
});